...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Cargar dataset de Blizzard si existe (construido en deploy)
    ...
    // Envío del asistente: sugerir contras
    if (assistantForm && assistantResults) {
        assistantForm.addEventListener("submit", e => {
            e.preventDefault();
            const nameInput = document.getElementById("petTarget");
            ...
            const detected = targetName
                ? `<p>Objetivo: <strong>${escapeHtml(targetName)}</strong> ${petCatalog[normalizeKey(targetName)] ? `(tipo detectado: ${labelForType(targetType)})` : `(tipo indicado: ${labelForType(targetType)})`}</p>`
                : `<p>Tipo objetivo: <strong>${labelForType(targetType)}</strong></p>`;
    ...
    (function setupCharacterLookup(){
        ...
        function onAuthMessage(ev) {
            try {
                ...
                const who = (data.characterName && data.realmSlug) ? `${escapeHtml(data.characterName)} @ ${escapeHtml(data.realmSlug)}` : '';
